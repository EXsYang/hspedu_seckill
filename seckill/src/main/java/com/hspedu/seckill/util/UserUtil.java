...
public class UserUtil {
    public static void create(int count) throws Exception {
        ...
        for (int i = 0; i < users.size(); i++) {
            ...
            //设置输入网页密码（相当于输出到页面）
            co.setDoOutput(true);
            // co.setConnectTimeout(10000); // 连接超时1.5秒
            // co.setReadTimeout(15000); // 读取超时1.5秒
            //这里需要连接是可以得到的，即SeckillApplication是运行状态,确认服务器正在运行，并且网络连接没有问题。
            ...
            //得到userTicket,后端的返回中需要设置返回的数据 即需要设置要返回的 票据ticket
